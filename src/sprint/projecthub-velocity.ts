// PROJECTHUB scope (学生 PJ) の velocity 代替算出。
//
// docs/design/projecthub-pm.md H1 最終裁定 (2026-07-17, 2): 学生 PJ の velocity は Memoria
...
export interface ProjectHubVelocityFallback {
  projectRef: string;
  category: '*';
  kFactor: 1;
  throughput: number;
  ...
  sampleSize: number;
  source: 'projecthub_completed_throughput' | 'projecthub_cold_start_capacity';
}

function sumEffort(tasks: ProjectHubSprintTask[]): number {
  return tasks
...
export function calculateProjectHubVelocityFallback(
  projectRef: string,
  tasks: ProjectHubSprintTask[],
  now: Date,
): ProjectHubVelocityFallback {
  ...
  if (completedEffort > 0) {
    return {
      ...
      source: 'projecthub_completed_throughput',
    };
  }
  ...
  return {
    ...
    distribution: WIDE_DISTRIBUTION,
    sampleSize: 0,
    source: 'projecthub_cold_start_capacity',
  };
}
