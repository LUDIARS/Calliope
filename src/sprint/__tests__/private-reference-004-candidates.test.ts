...
describe('build<private-reference-004>Candidates', () => {
  it('excludes completed tasks and human-gate (goal) tasks', () => {
    const result = build<private-reference-004>Candidates([
  ...
  it('marks tasks without a resolved priority instead of silently scoring them', () => {
    const result = build<private-reference-004>Candidates([task({ taskRef: 'actio:1' })], [], []);
    expect(result.missingPriorities).toEqual(['actio:1']);
    expect(result.candidates[0]?.priority).toBe(0);
  });

  it('always treats <private-reference-004> candidates as ready (H1: dependency graph deferred)', () => {
    const result = build<private-reference-004>Candidates([task({ taskRef: 'actio:1' })], [], [{ ref: 'actio:1', resolvedScore: 5 }]);
