...
import type { ProjectHubProjectProgress } from '../sprint/progress.ts';
...
export function makeNuntiusClient(options: { baseUrl: string; token: string }) {
  ...
  return {
    ...
    // docs/design/projecthub-pm.md H4: PROJECTHUB (学生 PJ) 向け週次進捗レポート配信。
    publishProjectHubWeeklyReport: (report: { generatedAt: string; projects: ProjectHubProjectProgress[] }) =>
      publish('calliope.projecthub.weekly', 'projecthub_weekly_report', 'report', report),
  };
}
